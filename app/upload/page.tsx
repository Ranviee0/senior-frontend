...
const UploadPage = () => {
  ...
  const [formDataToSubmit, setFormDataToSubmit] =
    useState<UploadFormValues | null>(null);
  ...
  const steps = [
    ...
    },
    {
      ...
      description: "Nearby development information",
    },
  ...
  const validateStep = async () => {
    ...
    switch (currentStep) {
      case 0:
        ...
        if (!isStepValid) {
          if (errors.land_name)
            newErrors.push(errors.land_name.message as string);
          if (errors.description)
            newErrors.push(errors.description.message as string);
      ...
      case 1:
        ...
        if (!isStepValid) {
          ...
          if (errors.latitude)
            newErrors.push(errors.latitude.message as string);
          if (errors.longitude)
            newErrors.push(errors.longitude.message as string);
          if (errors.zoning) newErrors.push(errors.zoning.message as string);
          if (errors.pop_density)
            newErrors.push(errors.pop_density.message as string);
          if (errors.flood_risk)
            newErrors.push(errors.flood_risk.message as string);
        }
        break;
      case 2:
  ...
  const areAllPreviousStepsValid = async () => {
    ...
    const step0Valid = await trigger(["land_name", "description", "area"], {
      shouldFocus: false,
    });
    ...
    const step2Valid = await trigger(["nearby_dev_plan"], {
      shouldFocus: false,
    });
    ...
    if (isStepValid) {
      ...
      if (currentStep === 2) {
        ...
        if (allValid) {
          ...
          setStepErrors([
            "Please complete all previous steps before setting the price",
          ]);
        }
      } else {
        setCurrentStep(Math.min(steps.length - 1, currentStep + 1));
  ...
  const submitForm = async () => {
    ...
    try {
      ...
      if (formDataToSubmit.zoning) {
        formData.append("zoning", formDataToSubmit.zoning);
      }
      ...
      setSubmissionStatus({
        success: true,
        message:
          "Land details uploaded successfully! Redirecting to home page...",
      });
